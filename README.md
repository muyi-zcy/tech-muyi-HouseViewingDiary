...
- 蒲公英：`https://www.pgyer.com/tech-muyi-HouseViewingDiary`
...
- **统计概览**：房源总数、各状态数量及「已定/总记录」占比；支持**看房日历**（按月热力格，点某天聚合查看当日记录）。
- **数据归档（多档案）**：将当前数据一键归档并清空，可在不同归档间切换（适合多城市/多用户），并支持删除归档。
- **界面结构**：底部三栏 **看房记录**、**统计**、**地图**，Material 3 与新拟态风格。

### 数据与隐私

数据通过本机 **SharedPreferences** 持久化，日常以本地为主（支持归档/切换）。以下能力在使用时可能触发系统权限/网络请求：

- **相机/相册**：用于拍照、选图与本地媒体预览。
- **麦克风**：用于语音备注录制与回放。
- **定位**：用于地图选点/当前位置辅助（以实际系统授权为准）。
- **网络访问（用户主动触发）**：分享解析在需要“网页补全”时会请求房源详情页；房源详情/统计页也支持在应用内打开外部页面（WebView 或浏览器）。
...
- **统计**：总记录数、各状态分布、已定占比；提供**看房日历（月视图热力格）**与“按天聚合”的记录列表
- **分享解析**：粘贴平台分享文案或链接，自动解析并可选网页补全（`listing_share_parser`）；支持在详情页**应用内打开原分享链接**
- **经纪人信息**：记录中介姓名、电话，支持一键拨号
- **数据持久化**：本地存储（SharedPreferences），默认不依赖网络
- **数据归档**：归档当前数据、切换归档、删除归档（用于多城市/多用户切换）
...
| `http` | 分享链接网页补全等网络请求 |
| `intl` | 时间格式化等 |
| `package_info_plus` | 读取应用版本信息（设置页展示） |
...
数据以 JSON 形式存储在 `SharedPreferences` 中；支持“当前数据”与“归档数据”并存（归档元信息 + 归档数据/标签分别存储），首次启动会生成示例数据。
